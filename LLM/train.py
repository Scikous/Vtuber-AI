...
class ModelTrainer:
    def __init__(self, model_name: str, new_model_name: str, pad_token: str, output_dir: str, dataset_path: str):
        self.model_name = model_name
        self.new_model_name = new_model_name
        self.pad_token = pad_token
        self.output_dir = output_dir
        self.dataset = self.load_and_split_dataset(dataset_path)
        self.tokenizer = None
        self.model = None
    ...
    def train_model(self):
        ...
        trainer.train()
        trainer.save_model(self.new_model_name)
        print("Finished Training")

    
    def convert_and_save_model(self):
        tokenizer = AutoTokenizer.from_pretrained(self.new_model_name)
        model = AutoModelForCausalLM.from_pretrained(
            self.model_name,
            torch_dtype=torch.float16,
            device_map="cuda:0",
        ...
        model.resize_token_embeddings(len(tokenizer), pad_to_multiple_of=8)
        model = PeftModel.from_pretrained(model, self.new_model_name, offload_folder="LLM/offload")
        model = model.merge_and_unload()
...
if __name__ == "__main__":
    trainer = ModelTrainer(MODEL_NAME, NEW_MODEL, PAD_TOKEN, OUTPUT_DIR)
    trainer.train_model()
