...
def get_rand_token_len(min_tokens=15, max_tokens=100, input_len=0):
    ...
    token_weights = np.linspace(start=1.0, stop=0.05, num=adjusted_max_tokens-min_tokens)
...
def dialogue_generator(model, tokenizer, comment, prompt_template):
    ...
    def is_incomplete_sentence(text):
        return text.strip()[-1] not in {'.', '!', '?'} #reply will end on not ., !, ? clean it beforehand then check
    ...
    for attempt in range(max_attempts):
        ...
        # prompt = prompt_template(user_str=continuation_prompt, context_str=generated_text)
        # #print(prompt)
        # inputs = tokenizer(prompt, return_tensors="pt")
...
def model_loader(base_model_name="TheBloke/CapybaraHermes-2.5-Mistral-7B-GPTQ", custom_model_name=""):
    ...
    if custom_model_name:
        ...
        model = PeftModel.from_pretrained(model, custom_model_name, offload_folder="LLM/offload")
