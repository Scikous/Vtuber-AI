...
class TTS:
    ...
    def run(self, inputs:dict):
        ...
        try:
            ...
            # if not return_fragment:
            #     print("*@*()#$&$&"*35, len(batch_index_list), idx)
    ...
    def audio_postprocess(self, 
        ...
        ##taking only 1 piece of data no longer need for iterating through each batch
        # for i, batch in enumerate(audio):
        #     print("BATCHING", i,batch)
        for j, audio_fragment in enumerate(audio[0]):
            max_audio=torch.abs(audio_fragment).max()#简单防止16bit爆音
            if max_audio>1: audio_fragment/=max_audio
            audio_fragment:torch.Tensor = torch.cat([audio_fragment, zero_wav], dim=0)
            audio[0][j] = audio_fragment.cpu().numpy()
