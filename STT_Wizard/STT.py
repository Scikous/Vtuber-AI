...
#     def start_stream(self, sentence_callback, transcription_func=None, device_index: int = None):
#         """
#         Starts the audio stream and runs processing in a background thread.
...
#         print("Starting audio stream for STT...")
#         vad = webrtcvad.Vad(self.vad_aggressiveness)
#         audio_buffer = deque(maxlen=int(self.sample_rate * self.audio_window_s))
#         # Ensure we have a valid transcription function
...
#             try:
#                 # VAD logic is simplified here for clarity. Your original logic is fine.
#                 is_speech = vad.is_speech(
#                     (indata * 32767).astype(np.int16).tobytes(), self.sample_rate
#                 )
#                 audio_buffer.extend(indata.flatten())
                
#                 # Simple logic: if not speech and buffer has content, process it.
#                 if not is_speech and len(audio_buffer) > self.sample_rate * self.min_chunk_s_for_interim:
#                     audio_np = np.array(list(audio_buffer), dtype=np.float32)
#                     audio_buffer.clear() # Clear buffer immediately
                    
#                     transcript = transcription_func(audio_np, beam_size=5, temperature=0.0)
#                     if transcript:
#                         sentence_callback(transcript)
...
#     def listen_and_transcribe(self, callback, **kwargs):
#         pass

#     # def listen_and_transcribe(self, sentence_callback, transcription_func=None, device_index: int = None):
#     #     """Process audio input, detect speech, and transcribe it."""
#     #     vad = webrtcvad.Vad(self.vad_aggressiveness)
#     #     audio_buffer = deque(maxlen=int(self.sample_rate * self.audio_window_s))
#     #     transcription_func = transcription_func or self.transcribe_audio_sync
